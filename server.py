...
# Initialize event queue, database, publisher, AI agent, and consumer
event_queue: asyncio.Queue[dict] = asyncio.Queue()
db = ChatDatabase()
publisher = EventPublisher(event_queue)
ai_agent = MockedAIAgent(publisher)
consumer: AIEventConsumer | None = None
connected_clients: set = set()  # Set of connected WebSocket clients
...
async def lifespan(app: FastAPI):
    ...
    consumer = AIEventConsumer(event_queue, db, ai_agent, connected_clients)
...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    await handle_websocket_connection(websocket, db, publisher, connected_clients)
